<!-- Auto-generated by scripts/fetch-docs.ts from https://nextjs.org/docs/app/api-reference/config/next-config-js/serverActions (Next.js 16.3.5, fetched 2026-09-15). Do not edit. -->
...
Options for configuring Server Actions behavior in your Next.js application. For how Server Actions work, including the security boundary these options tune, see the [Server Actions guide](https://nextjs.org/docs/app/guides/server-actions).
...
For example, remote development through a tunnel needs the tunnel hostname in two places: in [`allowedDevOrigins`](https://nextjs.org/docs/app/api-reference/config/next-config-js/allowedDevOrigins), so the dev server serves its own assets and endpoints, and here, so actions from it are accepted:
